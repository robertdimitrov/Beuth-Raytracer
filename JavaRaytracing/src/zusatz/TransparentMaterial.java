	...
	public Color colorFor(Hit hit, World world, Tracer tracer) {
		...
		final Color reflected = tracer.trace(new Ray(p, hit.ray.d.normalized()
				.add(new Vector3(hit.n.x, hit.n.y, hit.n.z).mul(2 * Math
						.cos(cosPhi1)).normalized())));
		if (reflected != null) {
			color = color.add(reflected.mul(bigR));
		}
		
		//Transmission berechnen
		final Color transmitted = tracer.trace(new Ray(p, hit.ray.d.normalized().mul(
				eta1 / indexOfRefraction).add(
				new Vector3(hit.n.x, hit.n.y, hit.n.z).normalized().mul(cosPhi2
						- (eta1 / indexOfRefraction) * cosPhi1).mul(-1))));
		if (transmitted != null) {
			color = color.add(transmitted.mul(bigT));
		}
